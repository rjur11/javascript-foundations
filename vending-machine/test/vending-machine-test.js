...
describe("VendingMachine", function() {
  ...
  it("should not allow a selection of an item not in the vending machine", function() {
    var machine = new VendingMachine({id: 123, isBroken: true});
    var chips = new Snack("lay's sour cream and onion chips", 1.0);
    ...
    assert.equal(
      machine.purchaseSnack("milky way", 0.5),
      "Sorry, that snack is not in the vending machine. Please choose a valid snack."
    );
    assert.equal(machine.snacks[0].itemsInStock, 5);
  });
});
